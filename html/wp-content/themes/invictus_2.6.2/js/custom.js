...
/*-----------------------------------------------------------------------------------*/
	...
	jQuery.fn.addToMobileMenu = function(){
		
		var windowWidth = jQuery(window).width();
		
		if( windowWidth <= 979 ) {

			if( !jQuery('#mobile-menu-dropdown').length ) {				
				var $mobileNav = jQuery('<a id="mobile-menu-dropdown" href="#open-mobile-menu">Menu</a>');				
				$mobileNav.insertBefore('#page');
				jQuery(this).insertAfter('#mobile-menu-dropdown').wrap('<div id="mobile-menu-wrap" />');
				my_mobile_menu_listener();
			}
			
		} else {
			$myMenuClone.css('display', 'none');
		}
...
*/
	...
	if (jQuery().superfish) {
		...
		var $myMenuClone = jQuery('#navHolder > ul').clone().attr('id', 'primary-nav-mobile');	
		...
		$myMenuClone.addToMobileMenu();
	
		// Fire the event listener
		function my_mobile_menu_listener() {
			jQuery('#mobile-menu-dropdown').on(clickevent,function(e) {
				if( jQuery('body').hasClass('ie8') ) {
	
					var mobileMenu = jQuery('#primary-nav-mobile');
					...
					jQuery('#primary-nav-mobile').stop().slideToggle(500);
		...
		// add the submenu navigation link hover effect
		jQuery('#navigation ul ul li').on("hover", 'a', function(e) {			
			var a = jQuery(this);			
			if(e.type == "mouseenter") {				
				a.stop().animate({ right: -8 },250);								
			}else if (e.type == "mouseleave") {				
				a.stop().animate({ right: 0 },250);								
			}
		});			
	...
	if( jQuery('.portfolio-list').size() ) {
		...
		jQuery('.portfolio-list').livequery(function(){ 
			...
			jQuery(this).on('hover', 'li.item', function(e){ // add the hover effect to the images to show the magnifier and do the animation
				...
				// mouseenter
				if(e.type == "mouseenter") {
					var cap = target.not('.portfolio-list li.show-title').find('.item-caption');
					// target.not('.portfolio-sortable-grid li.item img, .portfolio-fullsize-grid li.item img, #flickrPortfolio li.item img').stop(true,true).animate({ top: "-="+3 });
					target.not('.portfolio-list li.no-hover').find('img').stop().animate({ opacity : 0.25 }, 250);
					cap.stop().animate({ top: target.not('.portfolio-list li.no-hover').find('img').outerHeight() - cap.outerHeight() }, 250);
				
				// mouseleave					
				}else if (e.type == "mouseleave") {
					var cap = target.not('.portfolio-list li.show-title').find('.item-caption');
					// target.stop(true,true).animate({ top : "+="+3 });
					target.not('.portfolio-list li.no-hover').find('img').stop().animate({ opacity : 1 }, 250)						
					cap.stop().animate({ top: "100%" }, 250);
				}
...
/*-----------------------------------------------------------------------------------*/
	...
	jQuery(window).load( function() { 
		...
		jQuery('#primary-nav-mobile').livequery(function(){
			
			jQuery(this).on(clickevent, 'a', function(e) {
				var a = jQuery(this);
				a.next('ul').stop().slideToggle(500);
				jQuery('#primary-nav-mobile ul.sub-menu').not(a.next('ul')).hide();
			})
	...
	function responsiveChanges(viewport) {
		...
		if ( viewport.width >= 768 && viewport.width <= 979){
			...
			if($sidebar.length){
				...
				$primary.not('#primary.template-fullsize-gallery').css({ marginTop: - $brand.outerHeight(true) });
			}
		...
		if ( viewport.width >= 980 ){			
			...
			if($sidebar.length){
				...
				$primary.not('#primary.template-fullsize-gallery').css({ marginTop: - $brand.outerHeight(true) + parseInt($main.css('padding-top')) - 10 });
			}
...
/*-----------------------------------------------------------------------------------*/
/*	13. =Add rel to prettyphoto images in content
/*-----------------------------------------------------------------------------------*/
	
	if( jQuery('.entry-content .gallery-item').size() > 0 ){		
		jQuery('.entry-content .gallery-item .gallery-icon a')
			.attr('data-link', jQuery( jQuery('.entry-content .gallery-item .gallery-icon a') ).attr('href') )
