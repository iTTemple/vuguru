(function($) {
	...
	perc_to_px = function(size, with_other) {
	...
	jQuery(window).scroll(function () {
		if( jQuery('#mg_overlay_content').size() > 0 ) {		
			...
			if( jQuery('#mg_full_overlay').height() > jQuery(window).height() && top_scroll == 0 && parseInt($item_content.css('margin-top')) < 0) {
				 setTimeout(function() {
					$item_content.stop().animate({ marginTop: "60px" }, 500, 'linear');
				}, 150);	
			}
		}
	});
